...
.. csv-table::
   ...
   "``void render(int16_t x, int16_t y, int16_t w, int16_t h, const Surface &dst, int16_t dstX = 0, int16_t dstY = 0)``", "画面領域 (x, y, w, h) を ``dst`` の (dstX, dstY) に描く。画面と ``dst`` の両方でクリップされる。``dst`` は RGB565_SWAPPED、RGB565、RGB444 のいずれか"
